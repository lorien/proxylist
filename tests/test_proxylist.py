...
from proxylist.source import LinesListProxySource
...
class ProxyListTestCase(TestCase):
    ...
    def test_constructor(self) -> None:
        plist = ProxyList(LinesListProxySource(["foo:88"]))
    ...
    def test_from_list(self) -> None:
        plist = ProxyList.from_lines_list(["foo:1", "bar:2"])
        self.assertEqual(plist.get_next_server().host, "foo")
    ...
    def test_iter_interface(self) -> None:
        plist = ProxyList.from_lines_list(["foo:1"])
